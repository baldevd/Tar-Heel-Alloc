...
void *malloc(size_t size) {
    ...
    // Delete the following two lines2
...
// new
    ...
    //old
    int bytes_per_object = 1<<(power + 5);
    struct object *cursor = (struct object *) ptr;
    

    cursor->next = bkeep->free_list;
    bkeep->free_list = cursor;
     ...
     */
    memset((&cursor->next) + 1, FREE_POISON, bytes_per_object - sizeof(struct object *));
    ...
    while (levels[power].whole_superblocks > RESERVE_SUPERBLOCK_THRESHOLD) {
        ...
        // if the freed block is the head.
        struct superblock_bookkeeping *head = levels[power].next;
        int max_count = (SUPER_BLOCK_SIZE / bytes_per_object) - 1;

        if(head->free_count == max_count) {
            levels[power].next = head->next;
            munmap(head, SUPER_BLOCK_SIZE);
            levels[power].whole_superblocks--;
            levels[power].free_objects -= max_count;

        } else {
            struct superblock_bookkeeping *ptr1 = levels[power].next;
            struct superblock_bookkeeping *ptr2 = levels[power].next->next;

            while( ptr2 != NULL) {
                if(ptr2->free_count == max_count) {
                    ptr1->next = ptr2->next;
                    munmap(ptr2, SUPER_BLOCK_SIZE);
                    levels[power].whole_superblocks--;
